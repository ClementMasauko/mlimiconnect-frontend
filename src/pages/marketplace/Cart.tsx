...
export default function Cart() {
  ...
  return (
    <div className="mx-auto max-w-7xl">
      <div className="mb-5 sm:mb-7"><p className="text-xs font-bold uppercase tracking-[.15em] text-green-700 sm:text-sm">{t("yourOrder")}</p><h1 className="mt-1 text-3xl font-extrabold tracking-tight dark:text-white">{t("shoppingCart")}</h1></div>
      <div className="grid gap-4 sm:gap-7 lg:grid-cols-[1fr_360px]">
        <div className="space-y-3">{items.map(item => (
          <Card key={item.product.id} className="grid grid-cols-[72px_1fr] gap-3 p-3 sm:flex sm:items-center sm:gap-4 sm:p-4">
            <img src={item.product.image} alt="" className="h-20 w-[72px] rounded-lg object-cover sm:w-24" />
            ...
            <div className="col-span-2 flex items-center rounded-lg border border-slate-200 sm:col-span-1 dark:border-gray-700"><button onClick={() => updateQuantity(item.product.id, item.quantity - 1)} className="grid min-h-11 min-w-11 place-items-center hover:bg-slate-100 dark:hover:bg-gray-800" aria-label={t("decreaseQuantity")}><Minus size={16} /></button><span className="w-8 text-center text-sm font-bold">{item.quantity}</span><button onClick={() => updateQuantity(item.product.id, item.quantity + 1)} className="grid min-h-11 min-w-11 place-items-center hover:bg-slate-100 dark:hover:bg-gray-800" aria-label={t("increaseQuantity")}><Plus size={16} /></button></div>
            <div className="col-span-2 flex items-center justify-between text-right sm:col-span-1 sm:block"><p className="font-extrabold text-slate-900 dark:text-white">MWK {(item.product.price * item.quantity).toLocaleString()}</p><button onClick={() => removeItem(item.product.id)} className="min-h-11 px-2 text-xs font-semibold text-red-600 hover:underline">{t("remove")}</button></div>
          </Card>
        ))}</div>
        <Card className="sticky bottom-16 h-fit p-4 sm:static sm:p-6">
