...
export default function BuyerDashboard() {
  ...
  useEffect(() => { if (demoEnabled) return; type OrderRow={id:number;total:string|number;status:string;created_at:string;items?:Array<{name?:string;seller?:string}>}; Promise.all([api.get("/api/dashboard/overview/"), api.get<{results?:OrderRow[]}>("/api/marketplace/orders/")]).then(([overview, orderResponse]) => { const summary = overview.data.stats; const rows=orderResponse.data.results||[]; setStats({ ordersPlaced: summary.ordersPlaced || 0, totalSpent: Number(rows.reduce((sum, order) => sum + Number(order.total), 0)), activeOrders: rows.filter(order => ["pending", "paid"].includes(order.status)).length, favoriteFarmers: 0 }); setOrders(rows.slice(0, 5).map(order => ({ id: `ORD-${order.id}`, product: order.items?.[0]?.name || "Marketplace order", farmer: order.items?.[0]?.seller || "Seller", amount: Number(order.total), status: order.status, date: new Date(order.created_at).toLocaleDateString() }))); }).catch(() => { setStats({ ordersPlaced: 0, totalSpent: 0, activeOrders: 0, favoriteFarmers: 0 }); setOrders([]); }); }, [demoEnabled]);
  return (
