...
export default function AccountSecurity() {
  ...
  const regenerateRecoveryCodes = async () => {
    setBusy(true); setMessage("");
    try { const { data } = await api.post<{ recovery_codes: string[] }>("/api/auth/2fa/recovery-codes/", { code: twoFactorCode, password }); setRecoveryCodes(data.recovery_codes); setTwoFactorCode(""); setPassword(""); await load(); setMessage("Old recovery codes were replaced."); }
    catch (reason) { setMessage(getApiError(reason, "Could not regenerate recovery codes.")); }
    finally { setBusy(false); }
  };
  ...
  return <div className="mx-auto max-w-6xl"><div className="mb-7"><p className="text-sm font-bold uppercase tracking-wider text-green-700">My account</p><h1 className="mt-1 text-3xl font-extrabold">Sign-in & security</h1><p className="mt-2 text-slate-500">Manage how you access MlimiConnect and review recent security activity.</p></div><div className="grid gap-7 lg:grid-cols-[240px_1fr]"><Card className="h-fit p-3"><AccountNav /></Card><div className="space-y-4">
    ...
    <Card className="p-6"><div className="flex items-center justify-between gap-3"><div><h2 className="flex items-center gap-2 text-lg font-extrabold"><ShieldCheck size={20} /> Authenticator two-factor authentication</h2><p className="mt-1 text-sm text-slate-500">Require a rotating code after password or Google sign-in.</p>{user?.twoFactorRequired && !summary?.two_factor_enabled && <p className="mt-2 text-sm font-semibold text-amber-700">Your role requires 2FA before privileged access can be enabled.</p>}</div>{summary?.two_factor_enabled && <span className="rounded-full bg-green-100 px-3 py-1 text-xs font-bold text-green-800">Enabled</span>}</div>
      ...
      {summary?.two_factor_enabled && !recoveryCodes.length && <div className="mt-5 space-y-3"><p className="text-sm text-slate-500">{summary.recovery_codes_remaining} recovery codes remain.</p>{summary.has_usable_password && <Input type="password" autoComplete="current-password" value={password} onChange={event => setPassword(event.target.value)} placeholder="Current password" />}<Input value={twoFactorCode} onChange={event => setTwoFactorCode(event.target.value.replace(/\D/g, ""))} inputMode="numeric" autoComplete="one-time-code" maxLength={6} placeholder="Current 6-digit authenticator code" /><div className="flex flex-wrap gap-2"><Button variant="outline" disabled={busy || twoFactorCode.length !== 6 || (summary.has_usable_password && !password)} onClick={() => void regenerateRecoveryCodes()}>Replace recovery codes</Button><Button variant="outline" disabled={busy || twoFactorCode.length !== 6 || (summary.has_usable_password && !password)} onClick={() => void disableTwoFactor()}>Disable two-factor authentication</Button></div></div>}
    </Card>
