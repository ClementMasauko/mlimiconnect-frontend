...
const cropKeys = { Maize: "maize", Tomatoes: "tomatoes", Groundnuts: "groundnuts", Soybeans: "maize" } as const;
...
export default function MarketFeed() {
  ...
  const filteredData = mockMarketData.map(d => ({
    date: d.date,
    price: d[cropKeys[selectedCrop as keyof typeof cropKeys]],
  }));
  ...
  const exportData = () => {
    const csv = ["Period,Price (MWK)", ...filteredData.map(({ date, price }) => `${date},${price}`)].join("\n");
    const link = document.createElement("a");
    link.href = URL.createObjectURL(new Blob([csv], { type: "text/csv;charset=utf-8" }));
    link.download = `${selectedCrop.toLowerCase()}-market-prices.csv`;
    link.click();
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={filteredData}>
                ...
                <Tooltip
                  ...
                  formatter={(value) => [`MWK ${Number(value ?? 0).toLocaleString()}`, "Price"]}
                />
                <Line
                  type="monotone"
                  dataKey="price"
