...
export default function DeliveryManagement() {
  ...
  return <div className="mx-auto max-w-5xl p-4 py-8">
    ...
    {!delivery ? <Card className="mt-6 p-6">
      ...
      <Card className="mt-6 p-6"><strong className="capitalize">{delivery.status.replaceAll("_", " ")}</strong><p className="mt-2">{delivery.pickup_location} → {delivery.delivery_location}</p><p className="mt-1 text-sm">Estimated straight-line distance: {delivery.distance_km} km</p><p className="text-sm">Current fee: MWK {delivery.delivery_fee} · {delivery.transporter || "Awaiting transporter"}</p><p className="mt-2 text-sm text-slate-500">Liability: {delivery.liability_rule}</p><p className="mt-3 text-xs"><a href="https://www.geoapify.com/" target="_blank" rel="noreferrer" className="underline">Geocoding by Geoapify</a><span aria-hidden="true"> · </span><a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noreferrer" className="underline">Map data © OpenStreetMap contributors</a></p></Card>
