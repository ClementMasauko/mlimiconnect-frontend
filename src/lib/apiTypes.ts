...
export interface PaymentTransaction { provider: string; transaction_reference: string; provider_transaction_id: string; status: string; settled_amount: string | number | null; reconciled_at: string | null }
export interface MarketplaceOrder {
  id: number; status: string; subtotal: string | number; total: string | number;
  payment_method: string; payment_transaction: PaymentTransaction | null; acceptance_deadline: string | null; cancellation_reason: string; created_at: string; items: OrderItem[];
