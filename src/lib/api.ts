...
async function ensureCsrfToken(forceRefresh = false) {
  const cookieToken = typeof document === "undefined" ? null : csrfTokenFromCookie(document.cookie);
...
type CsrfRetryConfig = InternalAxiosRequestConfig & { _csrfRetried?: boolean };

api.interceptors.response.use(response => response, async (error: AxiosError) => {
  const retryConfig = error.config as CsrfRetryConfig | undefined;
  if (retryConfig && !retryConfig._csrfRetried && isCsrfFailure(error.response?.status, error.response?.data)) {
    retryConfig._csrfRetried = true;
    retryConfig.headers.set("X-CSRFToken", await ensureCsrfToken(true));
