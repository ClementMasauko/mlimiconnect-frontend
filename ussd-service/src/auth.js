...
async function verifyPin({ phone, pin }) {
  ...
  try {
    ...
    const { data } = await axios.post(`${baseURL.replace(/\/$/, '')}/api/ussd/authenticate`, { phone, pin }, { timeout: Number.isInteger(timeout) && timeout > 0 ? timeout : 5000, headers: { 'X-USSD-Service-Key': serviceKey } });
